...
// ==================== THERMAL COMFORT FEEDBACK (ASHRAE 55-2020) ====================
var _thermalPageRating = 0;  // legacy compat
var _ashraeThermal = { tsv: null, tpv: null, tsatisfaction: null };

var _ashraeScaleLabels = {
    tsv: { '-3':'Cold', '-2':'Cool', '-1':'Slightly Cool', '0':'Neutral', '1':'Slightly Warm', '2':'Warm', '3':'Hot' },
    tpv: { '-1':'Cooler', '0':'No Change', '1':'Warmer' },
    tsatisfaction: { '-3':'Very Dissatisfied', '-2':'Dissatisfied', '-1':'Slightly Dissatisfied', '0':'Neutral', '1':'Slightly Satisfied', '2':'Satisfied', '3':'Very Satisfied' }
};

function selectASHRAE(metric, value) {
    _ashraeThermal[metric] = value;
    var scaleId = metric + '-scale';
    var selectedId = metric + '-selected';
    document.querySelectorAll('#' + scaleId + ' .ashrae-btn').forEach(function(btn) {
        btn.classList.toggle('active', parseInt(btn.getAttribute('data-value')) === value);
    });
...
function resetThermalForm() {
    _ashraeThermal = { tsv: null, tpv: null, tsatisfaction: null };
...
function submitThermalFeedback() {
    ...
    fetch('/api/occupancy/feedback', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            thermal_rating: _ashraeThermal.tsv + 4,  // shift to 1-7 for legacy compat
            tsv: _ashraeThermal.tsv,
            tpv: _ashraeThermal.tpv,
            tsatisfaction: _ashraeThermal.tsatisfaction,
            visual_rating: 0,
            comment: '[THERMAL] TSV=' + _ashraeThermal.tsv + ' TPV=' + _ashraeThermal.tpv + ' TSat=' + _ashraeThermal.tsatisfaction + (comment ? ' | ' + comment : ''),
    ...
    .then(function(result) {
        if (result.status === 'success') {
            showToast('Thermal feedback berhasil disimpan!', 'success');
            resetThermalForm();
            loadThermalFeedbackHistory();
        } else {
            showToast(result.message || 'Gagal menyimpan feedback', 'error');
        }
    })
...
function loadThermalFeedbackHistory() {
    ...
    .then(function(data) {
        ...
        var rows = (data.feedback || []).filter(function(item) {
            return item.feedback_type === 'thermal' || (item.tsv !== undefined) ||
                   (item.comment && item.comment.indexOf('[THERMAL]') >= 0);
        });
        if (rows.length === 0) {
            container.innerHTML = '<div style="color: var(--text-secondary); font-size: 13px; padding: 12px;">Belum ada thermal feedback.</div>';
            return;
        }
        var tsvLabels = {'-3':'Cold','-2':'Cool','-1':'Sl. Cool','0':'Neutral','1':'Sl. Warm','2':'Warm','3':'Hot'};
        var tpvLabels = {'-1':'Cooler','0':'No Change','1':'Warmer'};
        var tsatLabels = {'-3':'Very Dissatisfied','-2':'Dissatisfied','-1':'Sl. Dissatisfied','0':'Neutral','1':'Sl. Satisfied','2':'Satisfied','3':'Very Satisfied'};
        container.innerHTML = rows.slice(0, 20).map(function(item) {
            var tsv = item.tsv !== undefined ? item.tsv : null;
            var tpv = item.tpv !== undefined ? item.tpv : null;
            var tsat = item.tsatisfaction !== undefined ? item.tsatisfaction : null;
            var safeComment = (item.comment || '').replace(/</g,'&lt;').replace(/>/g,'&gt;');
            return '<div class="feedback-history-ashrae">' +
                '<div class="fh-header">' +
                '<span style="font-size:13px; font-weight:600;">Thermal Feedback</span>' +
                '<span style="font-size:11px; color:var(--text-secondary);">' + item.time + '</span>' +
                '</div>' +
                '<div class="fh-badges">' +
                (tsv !== null ? '<span class="fh-badge tsv">TSV: ' + (tsv >= 0 ? '+' : '') + tsv + ' ' + (tsvLabels[String(tsv)] || '') + '</span>' : '') +
                (tpv !== null ? '<span class="fh-badge tpv">TPV: ' + (tpv >= 0 ? '+' : '') + tpv + ' ' + (tpvLabels[String(tpv)] || '') + '</span>' : '') +
                (tsat !== null ? '<span class="fh-badge tsat">TSat: ' + (tsat >= 0 ? '+' : '') + tsat + ' ' + (tsatLabels[String(tsat)] || '') + '</span>' : '') +
                '</div>' +
                '<div style="font-size:12px; color:var(--text-secondary);">Occupancy: ' + (item.occupancy_count || 0) + ' person(s)</div>' +
                (safeComment ? '<div style="margin-top:6px; font-size:12px; opacity:0.75;">' + safeComment + '</div>' : '') +
                '</div>';
        }).join('');
    })
    .catch(function() {});
...
function exportThermalFeedback() {
    ...
    .then(function(data) {
        var rows = (data.feedback || []).filter(function(item) {
            return item.feedback_type === 'thermal' || (item.comment && item.comment.indexOf('[THERMAL]') >= 0);
        });
        if (rows.length === 0) { showToast('No thermal data to export', 'error'); return; }
        var csv = 'Time,TSV,TPV,Thermal Satisfaction,Occupancy,Comment\n';
        rows.forEach(function(r) {
            csv += r.time + ',' +
                   (r.tsv !== undefined ? r.tsv : '') + ',' +
                   (r.tpv !== undefined ? r.tpv : '') + ',' +
                   (r.tsatisfaction !== undefined ? r.tsatisfaction : '') + ',' +
                   (r.occupancy_count || 0) + ',"' + (r.comment || '').replace(/"/g, '""') + '"\n';
        });
        downloadCSV('thermal_feedback_ashrae.csv', csv);
...
// ==================== VISUAL COMFORT FEEDBACK (Visual Comfort Protocol) ====================
var _visualPageRating = 0;  // legacy compat
var _ashraeVisual = { lsv: null, lpv: null, vsav: null };

var _ashraeVisualLabels = {
    lsv:  { '-3':'Very dark', '-2':'Dark', '-1':'Slightly dark', '0':'Neutral', '1':'Slightly bright', '2':'Bright', '3':'Very bright' },
    lpv:  { '-1':'Darker', '0':'No change', '1':'Brighter' },
    vsav: { '-3':'Very dissatisfied', '-2':'Dissatisfied', '-1':'Slightly dissatisfied', '0':'Neutral', '1':'Slightly satisfied', '2':'Satisfied', '3':'Very satisfied' }
};

function selectASHRAEVisual(metric, value) {
    _ashraeVisual[metric] = value;
    var scaleId = metric + '-scale';
    var selectedId = metric + '-selected';
    document.querySelectorAll('#' + scaleId + ' .ashrae-btn').forEach(function(btn) {
        btn.classList.toggle('active', parseInt(btn.getAttribute('data-value')) === value);
...
function resetVisualForm() {
    _ashraeVisual = { lsv: null, lpv: null, vsav: null };
...
function submitVisualFeedback() {
    ...
    fetch('/api/occupancy/feedback', {
        ...
        body: JSON.stringify({
            thermal_rating: 0,
            visual_rating: _ashraeVisual.lsv + 4,  // shift to 1-7 for legacy compat
            lsv: _ashraeVisual.lsv,
            lpv: _ashraeVisual.lpv,
            vsav: _ashraeVisual.vsav,
            comment: '[VISUAL] LSV=' + _ashraeVisual.lsv + ' LPV=' + _ashraeVisual.lpv + ' VSaV=' + _ashraeVisual.vsav + (comment ? ' | ' + comment : ''),
    ...
    .then(function(result) {
        if (result.status === 'success') {
            showToast('Visual feedback berhasil disimpan!', 'success');
            resetVisualForm();
...
function loadVisualFeedbackHistory() {
    ...
    .then(function(data) {
        ...
        if (!container) return;
        var rows = (data.feedback || []).filter(function(item) {
            return item.feedback_type === 'visual' || (item.lsv !== undefined) ||
                   (item.comment && item.comment.indexOf('[VISUAL]') >= 0);
        });
        if (rows.length === 0) {
            container.innerHTML = '<div style="color: var(--text-secondary); font-size: 13px; padding: 12px;">Belum ada visual feedback.</div>';
            return;
        }
        var lsvLabels = {'-3':'V.Dark','-2':'Dark','-1':'Sl.Dark','0':'Neutral','1':'Sl.Bright','2':'Bright','3':'V.Bright'};
        var lpvLabels = {'-1':'Darker','0':'No Change','1':'Brighter'};
        var vsavLabels = {'-3':'V.Dissatisfied','-2':'Dissatisfied','-1':'Sl.Dissatisfied','0':'Neutral','1':'Sl.Satisfied','2':'Satisfied','3':'V.Satisfied'};
        container.innerHTML = rows.slice(0, 20).map(function(item) {
            var lsv  = item.lsv  !== undefined ? item.lsv  : null;
            var lpv  = item.lpv  !== undefined ? item.lpv  : null;
            var vsav = item.vsav !== undefined ? item.vsav : null;
            var safeComment = (item.comment || '').replace(/</g,'&lt;').replace(/>/g,'&gt;');
            return '<div class="feedback-history-ashrae">' +
                '<div class="fh-header">' +
                '<span style="font-size:13px; font-weight:600;">Visual Feedback</span>' +
                '<span style="font-size:11px; color:var(--text-secondary);">' + item.time + '</span>' +
                '</div>' +
                '<div class="fh-badges">' +
                (lsv  !== null ? '<span class="fh-badge lsv">LSV: '  + (lsv  >= 0 ? '+' : '') + lsv  + ' ' + (lsvLabels[String(lsv)]   || '') + '</span>' : '') +
                (lpv  !== null ? '<span class="fh-badge lpv">LPV: '  + (lpv  >= 0 ? '+' : '') + lpv  + ' ' + (lpvLabels[String(lpv)]   || '') + '</span>' : '') +
                (vsav !== null ? '<span class="fh-badge vsav">VSaV: ' + (vsav >= 0 ? '+' : '') + vsav + ' ' + (vsavLabels[String(vsav)] || '') + '</span>' : '') +
                '</div>' +
                '<div style="font-size:12px; color:var(--text-secondary);">Occupancy: ' + (item.occupancy_count || 0) + ' person(s)</div>' +
                (safeComment ? '<div style="margin-top:6px; font-size:12px; opacity:0.75;">' + safeComment + '</div>' : '') +
                '</div>';
...
function exportVisualFeedback() {
    ...
    .then(function(data) {
        var rows = (data.feedback || []).filter(function(item) {
            return item.feedback_type === 'visual' || (item.comment && item.comment.indexOf('[VISUAL]') >= 0);
        });
        if (rows.length === 0) { showToast('No visual data to export', 'error'); return; }
        var csv = 'Time,LSV,LPV,VSaV,Occupancy,Comment\n';
        rows.forEach(function(r) {
            csv += r.time + ',' +
                   (r.lsv  !== undefined ? r.lsv  : '') + ',' +
                   (r.lpv  !== undefined ? r.lpv  : '') + ',' +
                   (r.vsav !== undefined ? r.vsav : '') + ',' +
                   (r.occupancy_count || 0) + ',"' + (r.comment || '').replace(/"/g, '""') + '"\n';
        });
        downloadCSV('visual_feedback_ashrae.csv', csv);
